...
namespace CoolPropWasm
{
    namespace
    {
        ...
        val parameter_descriptor(CoolProp::parameters param)
        {
        ...
        bool is_js_array(const val &value)
        {
            return val::global("Array").call<bool>("isArray", value);
        }

        val to_js_array(const std::vector<double> &values)
        {
            val array = val::array();
        ...
        val axis_descriptor(CoolProp::parameters axis_key,
        ...
        std::vector<double> val_to_double_vector(const val &array_val)
        {
            std::vector<double> values;
        ...
        IsoRequest parse_isoline_request(const val &iso_val,
                                         const CoolProp::Plot::PropertyPlot &plot)
        {
            if (!iso_val.typeOf().strictlyEquals(val("object")))
            {
        ...
        std::vector<IsoRequest> collect_isoline_requests(const val &options,
                                                         const CoolProp::Plot::PropertyPlot &plot)
        {
            ...
            const val isolines_val = options["isolines"];
    ...
    val compute_property_plot(const val &options)
    {
        ...
        for (std::size_t i = 0; i < requests.size(); ++i)
        {
            ...
            val line_array = val::array();
            for (std::size_t j = 0; j < lines.size(); ++j)
            {
                const CoolProp::Plot::Isoline &line = lines[j];
                val line_obj = val::object();
    ...
    val list_supported_property_plots()
    {
        const auto &charts = chart_spec_map();
        val array = val::array();
        std::size_t idx = 0;
